...
/** POST /api/import/products — same shape as /api/import/customers; see that route's header comment. */
export async function POST(req: Request) {
  ...
  try {
    if (mode === "preview") {
      const preview = await previewImport(companyId, "products", csv);
      return NextResponse.json(preview);
    }
    const result = await commitImport(companyId, "products", csv);
