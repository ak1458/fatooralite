...
| N3 | WhatsApp integration | 16 | **D8 resolved — Option A; interim transport addendum 2026-08-19 (later same day)** | **PARTIAL — core delivery built via two providers, full 16-item scope not; no real send by either provider verified** | D8 approved Option A. `lib/whatsapp/send.ts` is now a provider dispatcher (Meta wins automatically once configured), not a single sender: `lib/whatsapp/providers/meta.ts` (Cloud API, media upload + approved-template message — the intended production/compliance-grade path, still deferred) and `lib/whatsapp/providers/openwa.ts` (a self-hosted gateway, added as a **temporary** interim transport by explicit owner instruction — **never the recommended production/compliance-grade choice**, see decision-register.md D8's addendum). `POST /api/invoices/:id/whatsapp` unchanged in every respect except a one-line provider-agnostic config check — recipient exclusively from `Customer.phone`, same anti-abuse design as N7's email route, for either provider. New: `GET /api/operator/whatsapp-session`, a read-only provider/session health check, same `OPERATOR_SECRET` pattern as D7. **Not delivered, honestly**: per-customer opt-in/consent tracking (M-308), delivery-status webhooks (M-306), Arabic-vs-English template selection (M-303/M-304), retry/fallback beyond one send attempt (M-312), any QR-pairing UI (deliberately not built — OpenWA's own dashboard covers it). Neither Meta nor OpenWA has had a real send verified this session — Meta stays owner-blocked on Business verification; OpenWA had no running instance to test against, so its 13 provider-level tests are mocked-fetch only |
